...
def calculate_demographic_data(print_data=False):
    ...
    country_salary_stats = df[df['salary'] == '>50K'].groupby('native-country')['salary'].count() / df.groupby('native-country')['salary'].count() * 100
    ...
    df_ocp = df[(df['native-country'] == 'India') & (df['salary'] == ">50K")]
    top_IN_occupation = df_ocp['occupation'].value_counts().idxmax()
